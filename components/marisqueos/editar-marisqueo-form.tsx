...
import {
  updateMarisqueoAction,
  getMarisqueoByIdAction,
...
interface EditarMarisqueoFormProps {
  marisqueo: Marisqueo;
  campaniaId: number;
  onSuccess?: () => void;
}

export function EditarMarisqueoForm({
  marisqueo,
  campaniaId,
  onSuccess,
}: EditarMarisqueoFormProps) {
  ...
  const [originalCaptura, setOriginalCaptura] = useState(0);
  const [marisqueoCompleto, setMarisqueoCompleto] = useState<any>(null);
  ...
  // Cargar datos completos del marisqueo cuando se abre el diálogo
  useEffect(() => {
    if (!isOpen) return;

    const loadMarisqueoData = async () => {
      setLoadingData(true);
      try {
        const result = await getMarisqueoByIdAction(marisqueo.id);
        if (result.error) {
          toast.error("Error al cargar datos del marisqueo");
          return;
        }

        setMarisqueoCompleto(result.data);
        setOriginalCaptura(result.data.n_captura);

        // Procesar coordenadas si existen
        let coordenadas = null;
        if (result.data.coordenadas) {
          coordenadas = aseguraCoordenada(result.data.coordenadas);
        }

        // Pre-rellenar el formulario
        form.setValue("transectaId", result.data.segmento.transecta.id);
        form.setValue("segmentoId", result.data.segmento_id);
        form.setValue("buzoId", result.data.buzo_id);
        form.setValue("nCaptura", result.data.n_captura);
        form.setValue("profundidad", result.data.profundidad || 0);
        form.setValue("tiempo", result.data.tiempo || 0);
        form.setValue("pesoMuestra", result.data.peso_muestra || 0);

        // Si tenemos coordenadas, las convertimos a sexagesimal
        if (coordenadas) {
          const sexagesimal = coordenadas.sexagesimal;
          form.setValue(
            "coordenadas.latitud.grados",
            sexagesimal.latitud.grados
          );
          form.setValue(
            "coordenadas.latitud.minutos",
            sexagesimal.latitud.minutos
          );
          form.setValue(
            "coordenadas.latitud.segundos",
            sexagesimal.latitud.segundos
          );
          form.setValue(
            "coordenadas.latitud.direccion",
            sexagesimal.latitud.direccion as "N" | "S"
          );
          form.setValue(
            "coordenadas.longitud.grados",
            sexagesimal.longitud.grados
          );
          form.setValue(
            "coordenadas.longitud.minutos",
            sexagesimal.longitud.minutos
          );
          form.setValue(
            "coordenadas.longitud.segundos",
            sexagesimal.longitud.segundos
          );
          form.setValue(
            "coordenadas.longitud.direccion",
            sexagesimal.longitud.direccion as "E" | "O"
          );
        }
      } catch (error) {
        toast.error("Error al cargar datos del marisqueo");
  ...
  // Verificar si ya existe un marisqueo con el número de captura (solo si cambió)
  const checkCapturaDisponible = async (
    ...
    // Si es el mismo número de captura original, no verificar
    if (nCaptura === originalCaptura) {
      setCapturaDisponible(true);
      return;
    }
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      const result = await updateMarisqueoAction(marisqueo.id, {
      ...
      toast.success("Marisqueo actualizado correctamente");
      onSuccess?.();
      setIsOpen(false);
    } catch (error: any) {
      toast.error(error.message || "Error al actualizar el marisqueo");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="min-w-fit">
        <DialogHeader>
          <DialogTitle>Editar Marisqueo</DialogTitle>
        </DialogHeader>

        {loadingData ? (
          <div className="flex items-center justify-center p-6">
            <p>Cargando datos del marisqueo...</p>
          </div>
        ) : (
          <Form {...form}>
